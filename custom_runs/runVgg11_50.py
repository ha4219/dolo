...
def main():
    ...
    loaders = create_dataloader(_in=_in, batch_size=64)

    device = 'cuda:1' if torch.cuda.is_available() else 'cpu'

    model.to(device)

    # optimizer = torch.optim.SGD(model.parameters(), lr=1e-6, momentum=0.1, weight_decay=0.001)
